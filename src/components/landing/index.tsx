...
function Navigation() {
  ...
  useEffect(() => {
    ...
    const fn = () => {
      ...
      // Progress bar + scrolled style — React state OK (pas critique)
      setScrollProgress(docH > 0 ? Math.min(scrollY / docH, 1) : 0);
      setScrolled(scrollY > 30);

      // Hide/show — DOM direct, pas de React render
      if (scrollY < 50) {
      ...
      } else if (scrollY < lastY.current - 5) {
        header.style.top = '14px';
        header.style.pointerEvents = 'auto';
      }
...
function MiniCV({ id = 'student', color = '#B76E6E' }: { id?: TemplateId; color?: string }) {
  if (id === 'student') return (
    <svg viewBox="0 0 130 168" style={{ width:'100%', height:'100%', display:'block' }}>
      ...
      <rect width="42" height="168" rx="7" fill={color}/>
      <rect x="7" width="35" height="168" fill={color}/>
      ...
      {[66,73,80,87,94,101,108,115].map((y,i) => (
        <rect key={y} x="7" y={y} width={[28,22,26,20,28,24,18,26][i]} height="5" rx="2.5"
      ...
      <rect x="62" y="34.5" width="28" height="5" rx="2" fill="#333"/>
      {[44,50,55,60].map((y,i) => <rect key={y} x="50" y={y} width={[68,54,62,50][i]} height="2.5" rx="1" fill="#eee"/>)}
      <circle cx="52" cy="72" r="5" fill={color}/>
      <rect x="62" y="69.5" width="22" height="5" rx="2" fill="#333"/>
      {[80,95,110].map(y => (
        <g key={y}>
          <circle cx="54" cy={y+2} r="3" fill={color}/>
          <rect x="60" y={y} width="50" height="3.5" rx="1.5" fill={color} opacity="0.55"/>
          <rect x="60" y={y+6} width="60" height="2.5" rx="1" fill="#e8e8e8"/>
          <rect x="60" y={y+11} width="52" height="2.5" rx="1" fill="#e8e8e8"/>
        </g>
      ))}
    </svg>
  );
  ...
  if (id === 'health') return (
    <svg viewBox="0 0 130 168" style={{ width:'100%', height:'100%', display:'block' }}>
      <rect width="130" height="168" rx="7" fill="white"/>
      <rect width="130" height="44" rx="7" fill={color}/>
      <rect x="0" y="20" width="130" height="24" fill={color}/>
      ...
      <rect x="46" y="10" width="48" height="7" rx="2" fill="rgba(255,255,255,0.9)"/>
      <rect x="46" y="21" width="32" height="3" rx="1.5" fill="rgba(255,255,255,0.55)"/>
      <rect x="100" y="10" width="22" height="2" rx="1" fill="rgba(255,255,255,0.38)"/>
      <rect x="100" y="15" width="18" height="2" rx="1" fill="rgba(255,255,255,0.28)"/>
      <rect x="100" y="20" width="20" height="2" rx="1" fill="rgba(255,255,255,0.28)"/>
      <rect width="130" height="3" y="44" fill={color} opacity="0.28"/>
      {[54,96].map((y,si) => (
        <g key={y}><circle cx="10" cy={y+3} r="6" fill={color}/>
        <rect x="20" y={y+1} width={[26,30][si]} height="4.5" rx="2" fill="#222" opacity="0.8"/></g>
      ))}
      {[63,75,105,117].map(y => (
        <g key={y}>
          <rect x="8" y={y} width="10" height="2" rx="1" fill="#ddd"/>
          <circle cx="22" cy={y+1} r="3" fill={color} opacity="0.7"/>
          <rect x="28" y={y} width="46" height="3.5" rx="1.5" fill={color} opacity="0.55"/>
          <rect x="28" y={y+6} width="50" height="2.5" rx="1" fill="#ececec"/>
        </g>
      ))}
      <rect x="84" y="48" width="46" height="120" fill="#F7F9F8"/>
      <rect x="84" y="48" width="1" height="120" fill="#eee"/>
      <rect x="88" y="55" width="30" height="2.5" rx="1" fill={color} opacity="0.7"/>
      {[62,68,74,80,86,92,98].map((y,i) => (
        <rect key={y} x="88" y={y} width={[26,20,28,22,24,18,26][i]} height="4.5" rx="2.2"
          fill="white" stroke={color} strokeWidth="0.5" strokeOpacity="0.5"/>
      ))}
      <rect x="88" y="110" width="30" height="2.5" rx="1" fill={color} opacity="0.7"/>
      {[117,125,133].map((y,i) => (
        <rect key={y} x="88" y={y} width={[34,28,32][i]} height="3" rx="1.5" fill="#e0e0e0"/>
      ))}
    </svg>
  );
...
function TemplatesSection() {
  ...
  const templates: Array<{ id: TemplateId; name: string; desc: string; color: string }> = [
    { id: 'student',   name: 'Étudiant',  desc: 'Sidebar colorée, moderne',  color: '#4A90E2' },
    ...
    { id: 'health',    name: 'Santé',     desc: 'Header coloré, médical',     color: '#52B788' },
